#' Overwrites Stms textprocessor to allow for custom stopword removal 
#' after stemming. 
...
textProcessor <- function(documents, metadata=NULL, 
                          ...
                          customstopwords=NULL, custompunctuation=NULL, v1=FALSE, customstemmedstops=NULL) {
  ...
  if(!is.null(customstemmedstops)) {
    if(verbose) cat("Remove customstemmedstops post stemming...\n")
    txt <- tm::tm_map(txt, tm::removeWords, customstemmedstops)
  }
  if(!is.null(metadata)) {
